function [Pf_SuS,delta_SuS,prob,b,Pf,b_line,Pf_line,samplesU,gruns] = SuS_poly(N, p0, ff,n,off,seed_number)
...
%% Pick your advanture which function
...
for i = 1:N
   ...
   if i < Naprox 
    ...
    if off==1
        ...
        [Z0,eplus] = start_approx(u_j(:,i),u_j(:,1:i-1),geval(1:i-1),[],G_LSF);
        
        if eplus<=0.1 % check uncertainty in prediction
...
while true 
   ...
   if j>1 && off==1 && b(j) > 0         % correction intermediate level
       ...
       [u_j,geval,brun,totalu,totalg] = blevel_fix(u_j,geval,G_LSF,N,p0,totalu,totalg);
   ...
   if b(j)>0
       ...
       [u_j, geval, lambda, sigma, acc,run,err1,totalu,totalg] = aCSback(N, lambda, b(j), b(1:j), rnd_seeds, rnd_geval,G_LSF,totalu,totalg,j,off); 
